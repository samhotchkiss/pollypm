...
from promptmaster.models import KnownProject, ProjectKind, PromptMasterConfig, ProviderKind
...
from promptmaster.projects import discover_recent_git_repositories, ensure_project_scaffold, make_project_key
...
class OnboardingApp(App[Path | None]):
    ...
    def __init__(self, config_path: Path, force: bool = False) -> None:
        ...
        if config_path.exists():
            try:
                ...
                open_permissions_by_default = existing.promptmaster.open_permissions_by_default
        ...
        if not installed:
            stage.mount(
                Panel(
                    ...
                    "Install Claude CLI or Codex CLI, then rerun `promptmaster`.",
    ...
    def _build_config(self) -> PromptMasterConfig:
