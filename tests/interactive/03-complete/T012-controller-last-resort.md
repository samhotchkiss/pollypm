...
Config: controller=claude_claude_swh_me, failover=[codex_s_swh_me]. _controller_candidates() tries controller first, then failover. Workers use non-controller accounts by default (_candidate_accounts selects from failover list first).

### Re-test — 2026-04-10 (via Polly)

Workers run on codex_s_swh_me (failover account), not the controller. Controller (claude) used only for heartbeat+operator. Workers would only get controller account if codex was exhausted. ✅
