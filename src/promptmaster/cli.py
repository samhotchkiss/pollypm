...
from promptmaster.account_tui import AccountsApp
...
from promptmaster.messaging import close_message, create_message, list_open_messages
...
def _install_global_promptmaster(root_dir: Path) -> tuple[bool, str]:
...
def _require_promptmaster_session(supervisor: Supervisor) -> None:
...
def _first_run_setup_and_launch(config_path: Path) -> None:
    ...
    _install_global_promptmaster(path.parent)
    up(config_path=path)
...
def onboard(
    ...
    installed, install_output = _install_global_promptmaster(path.parent)
    typer.echo("")
    typer.echo(f"Wrote onboarding config to {path}")
    if installed:
        typer.echo("Installed global commands: `promptmaster` and `pm`.")
    else:
...
def status(
    ...
    if supervisor.config.promptmaster.failover_accounts:
        typer.echo(
            "- failover_order="
            + ", ".join(_account_label(supervisor, name) for name in supervisor.config.promptmaster.failover_accounts)
        )
    else:
