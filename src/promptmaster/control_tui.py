...
class PromptMasterApp(App[None]):
    ...
    BINDINGS = [
        ...
        Binding("u", "ensure_promptmaster", "Ensure Session"),
    ...
    def _refresh_dashboard(self, supervisor: Supervisor, launches, windows, alerts, leases, errors) -> None:
        ...
        controller = supervisor.config.promptmaster.controller_account
        ...
        failover = ", ".join(supervisor.config.promptmaster.failover_accounts) or "none"
    ...
    def _hero_render(self, supervisor: Supervisor, launches, windows, alerts, leases) -> object:
        ...
        return Panel(
            ...
            border_style="#566574",
            subtitle=(
                f"Controller: {supervisor.config.accounts[supervisor.config.promptmaster.controller_account].email or supervisor.config.promptmaster.controller_account}"
    ...
    def _refresh_accounts(self, supervisor: Supervisor) -> None:
        controller = supervisor.config.promptmaster.controller_account
        failover = set(supervisor.config.promptmaster.failover_accounts)
        rows = [
            (
                (
                    account.key,
    ...
    def _account_detail(
        ...
        lines = [
            ...
            f"Controller: {'yes' if key == supervisor.config.promptmaster.controller_account else 'no'}",
            f"Failover: {'yes' if key in supervisor.config.promptmaster.failover_accounts else 'no'}",
        ]
    ...
    def _cockpit_detail(self, supervisor: Supervisor, config, key: str | None) -> str:
        ...
        if key == "settings":
            ...
            lines = [
                ...
                f"Open permissions by default: {'on' if supervisor.config.promptmaster.open_permissions_by_default else 'off'}",
                f"Controller account: {supervisor.config.promptmaster.controller_account}",
                f"Failover order: {', '.join(supervisor.config.promptmaster.failover_accounts) or 'none'}",
                "",
                "Secondary views:",
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        actions = {
            ...
            "dashboard-ensure": self.action_ensure_promptmaster,
    ...
    def action_ensure_promptmaster(self) -> None:
        def _ensure() -> None:
