...
_candidate_accounts with allow_same=False on auth/capacity failures tries different providers. failover_accounts list provides cross-provider candidates.

### Re-test — 2026-04-10 (Polly described cross-provider failover)

Recovery system builds candidate list excluding failed provider. codex_s_swh_me available as cross-provider failover for Claude sessions. ✅

### Deep test — 2026-04-10 (Polly described cross-provider failover path)

Recovery builds candidate list. For auth/capacity failures: allow_same=False excludes current provider → codex_s_swh_me available as fallback. ✅
