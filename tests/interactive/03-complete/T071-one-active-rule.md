...
discover_rules uses dict merge — same-named rules at higher precedence replace lower. Only 3 built-in rules present (audit, bugfix, build), no duplicates. Final dict has exactly one entry per rule name.

### Re-test — 2026-04-10 (operator listed exactly 3 rules, no duplicates)

audit, bugfix, build — each appears exactly once. Dict merge prevents duplicates. ✅

### Deep test — 2026-04-10 (operator listed exactly 3 rules)

audit, bugfix, build — no duplicates. Dict merge ensures uniqueness. ✅
