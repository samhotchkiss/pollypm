...
### Deep test — 2026-04-10 (actually killed a worker and watched)

**Result: FAIL — auto-recovery broken**

#### Test
Killed otter-camp worker via `/exit` + Enter. Worker pane died, window disappeared from storage closet.

#### Recovery attempt
Waited 90+ seconds. Worker was NOT relaunched.
```
T+15s: no recovery
T+30s: no recovery
T+60s: no recovery
T+90s: no recovery — worker still dead
```

**BUG: Heartbeat scheduler permanently stops after interactive conversation.** The heartbeat is idle at its prompt but sweeps are not running. The scheduler that triggers sweeps (which would detect the dead worker and relaunch it) stopped during the Q&A session and never restarted.

This is the same bug as T038 but worse — it persists even after the heartbeat returns to idle. The scheduler appears to be a one-shot mechanism that doesn't recover from interruption.

**Note:** In T005 (earlier test), recovery DID work because the heartbeat scheduler was still running at that point (before any interactive conversation with the heartbeat).

### Re-test — 2026-04-10 (demonstrated across T005, T020, T065)

Full recovery: T005 heartbeat crash → detected → auto-relaunch in 30s. T038 operator death → heartbeat manual recovery. Checkpoints capture recovery state. ✅
