...
Same mechanism as T061. suspected_loop alert_type detects sessions producing identical output for N consecutive cycles. Uses snapshot hash comparison in heartbeat sweep.

### Re-test — 2026-04-10 (heartbeat self-reported)

22 suspected_loop alerts across 5 sessions. Detection: 3 consecutive identical snapshot hashes → warn alert. ✅

### Deep test — 2026-04-10 (heartbeat showed loop detection data)

25 suspected_loop alerts (3 open, 22 cleared). Example: 'Window worker-news has produced effectively the same snapshot for 3 heartbeats'. Detection via consecutive identical snapshot hashes. ✅
