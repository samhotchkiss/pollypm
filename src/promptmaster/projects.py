...
def project_promptmaster_dir(project_path: Path) -> Path:
    return normalize_project_path(project_path) / ".promptmaster"


def project_dossier_dir(project_path: Path) -> Path:
    return project_promptmaster_dir(project_path) / "dossier"


def project_logs_dir(project_path: Path) -> Path:
    return project_promptmaster_dir(project_path) / "logs"


def project_artifacts_dir(project_path: Path) -> Path:
    return project_promptmaster_dir(project_path) / "artifacts"
...
def project_worktrees_dir(project_path: Path) -> Path:
    return project_promptmaster_dir(project_path) / "worktrees"
...
def ensure_project_scaffold(project_path: Path) -> Path:
    promptmaster_dir = project_promptmaster_dir(project_path)
    for directory in [
        promptmaster_dir,
    ...
    _ensure_gitignore_entry(normalize_project_path(project_path), ".promptmaster/")
    return promptmaster_dir
