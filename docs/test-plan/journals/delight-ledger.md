...
|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 (h0) | morning brief | M2 | 2 (self-narrating care) | `dashboard.briefing` = **null** — no "while you were away" narration at all; operator gets no chief-of-staff brief | `internal-comms` | #2473→#2477→#2486→#2490 | a929fbf69 | live brief text (cycle 71) | **YES (1→3.5/5)** | 4/5 | **shipped+improving (cycle 71): null→warm brief→M3 care narration (#2486)→First-up FIXED (#2490).** Brief now: "Progress: 129 commits… While you were away, I handled this: [recovery narration]… First up: savethenovel has a task stalled in progress. 4 inbox items waiting; open Inbox and clear that first." First-up is now a LIVE-project clean SENTENCE + recommendation (was a clipped raw alert since cycle 13). Residual (#2491): narration still names a synthetic test-proj (liveness signal classifies it live); no per-project ship story yet. M2 ~2.5→3.5. |
