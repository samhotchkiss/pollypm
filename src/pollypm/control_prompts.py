...
def operator_prompt() -> str:
    return (
        ...
        "and modular testable changes. When asked to look at or check on a project, go straight to the "
        "project registry to find its path and active worker session, then evaluate the project's file "
        "and git state and read the current session log. Do not broadly explore PollyPM internals unless "
        "specifically asked. If the user asked only for an end result, you may redirect method "
