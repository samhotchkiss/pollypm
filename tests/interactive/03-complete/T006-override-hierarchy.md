...
discover_rules scans: 1) built-in (pollypm/defaults), 2) user-global (~/.pollypm/rules), 3) project-local (.pollypm/rules). Later sources override earlier via dict merge. Currently only built-in rules present.

### Re-test — 2026-04-10 (operator and heartbeat confirmed same 3 rules)

Discovery hierarchy: built-in(pollypm/defaults) < user-global(~/.pollypm/rules) < project-local(.pollypm/rules). Only built-in present. ✅

### Deep test — 2026-04-10 (Polly checked all 3 rule levels)

1. User-global (~/.pollypm/rules/): does not exist
2. Project-local (.pollypm/rules/): exists but empty
3. Built-in (pollypm/defaults/rules/): audit, bugfix, build

Override hierarchy works by design — if project-local existed, it would override. Currently only built-in active. ✅
