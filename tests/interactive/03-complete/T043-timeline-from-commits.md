...
build_timeline() aggregates from multiple sources: git commits, JSONL files, doc files, config files. _git_commits_to_timeline() reads git log chronologically.

### Re-test — 2026-04-10

build_timeline aggregates from git commits, JSONL, docs, config files. _git_commits_to_timeline reads git log chronologically. ✅

### Deep test — 2026-04-10 (worker ran git log timeline check)

build_timeline in history_import.py (937 lines) aggregates git commits, JSONL, docs, config. Worker confirmed git log works from the project dir. ✅
