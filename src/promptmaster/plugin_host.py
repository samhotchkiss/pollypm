...
from promptmaster.plugin_api.v1 import HookContext, HookFilterResult, PromptMasterPlugin

PLUGIN_MANIFEST = "promptmaster-plugin.toml"
...
class ExtensionHost:
    def __init__(self, root_dir: Path) -> None:
        ...
        self._plugins: dict[str, PromptMasterPlugin] | None = None

    def plugins(self) -> dict[str, PromptMasterPlugin]:
    ...
    def _load_plugins(self) -> dict[str, PromptMasterPlugin]:
        loaded: dict[str, PromptMasterPlugin] = {}
    ...
    def _plugin_search_paths(self) -> list[tuple[str, Path]]:
        ...
        user = Path.home() / ".config" / "promptmaster" / "plugins"
        repo = self.root_dir / ".promptmaster" / "plugins"
        return [
            ("builtin", builtins),
            ("user", user),
            ("repo", repo),
    ...
    def _load_plugin_from_manifest(self, manifest: PluginManifest) -> PromptMasterPlugin | None:
        ...
        try:
            ...
            return None
        if not isinstance(plugin_obj, PromptMasterPlugin):
            self.errors.append(f"Plugin {manifest.name} entrypoint did not return PromptMasterPlugin")
            return None
        return plugin_obj
    ...
    def _load_module(self, manifest: PluginManifest) -> types.ModuleType:
        ...
        spec = spec_from_file_location(f"promptmaster_plugin_{manifest.name}_{manifest.source}", module_path)
