...
### Re-test — 2026-04-10 (operator self-reported prompt components)

Asked operator to list its initial prompt sections:
```
1. Persona/Role — "Polly, the PollyPM project manager." Behaviors: oversee, clarify, pick providers, launch/steer workers, detect drift.
2. Inbox monitoring directive — Monitor .pollypm/inbox/open/, own triage, route to PA.
3. Available Rules — audit, bugfix, build (with trigger conditions)
4. Available Magic — deploy-site, visual-explainer (with trigger conditions)
```

Operator noted: "No explicit project context block, no account roster — learned those by reading pollypm.toml during conversation."

**Gap:** The test expects 6 components but only 4 are in the initial prompt. Missing: explicit project context section, task/issue context. Workers start with empty prompts (no components at all).
