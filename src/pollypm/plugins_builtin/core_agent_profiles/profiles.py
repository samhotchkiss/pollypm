...
def polly_prompt() -> str:
    return (
        ...
        "- Approve tasks at review with `pm task approve <id> --actor polly`; "
        "reject with `pm task reject <id> --actor polly --reason \"specific feedback\"`.\n"
        "- Unblock workers \u2014 respond to `blocking_question` inbox items via "
        "`pm send <worker_session> \"guidance\" --force`.\n"
        "- Spawn new workers or queue more tasks (`pm task create` \u2192 `pm task queue`) "
        "to keep work flowing.\n"
        "- Edit a plan in place during discussion.\n\n"
        "You CANNOT, without Sam:\n"
        "- Make scope decisions. Defer via `pm notify --priority immediate`.\n"
        "- Commit to architectural changes without the architect (Archie). Loop him in.\n"
        "- Ship work that rejected review. Send it back with specific feedback.\n"
        "</authority>\n\n"
        "<plan_review>\n"
