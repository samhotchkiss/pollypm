...
### Re-test — 2026-04-10 (via heartbeat querying its own DB)

Heartbeat reported 15 tables with row counts:
```
│ events             │ 716  │ Lifecycle events          │
│ heartbeats         │ 784  │ Heartbeat cycles          │
│ checkpoints        │ 782  │ Session snapshots         │
│ token_samples      │ 142  │ Token usage raw           │
│ token_usage_hourly │ 56   │ Aggregated usage          │
│ alerts             │ 48   │ Health alerts             │
│ sessions           │ 5    │ Session config            │
│ session_runtime    │ 6    │ Runtime state             │
│ worktrees          │ 3    │ Git worktree tracking     │
│ account_runtime    │ 2    │ Account health            │
│ memory_entries     │ 0    │ (unused)                  │
│ memory_summaries   │ 0    │ (unused)                  │
```
2 schema migrations found:
1. `ALTER TABLE sessions ADD COLUMN project` — multi-project support
2. `ALTER TABLE heartbeats ADD COLUMN snapshot_hash` — dedup hashing

Data survived both migrations with no loss. ✅
