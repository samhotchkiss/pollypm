...
Asked operator: "What rules and magic capabilities are available to you?"
```
Rules (specialized instructions):
│ audit  │ Code audit checklist           │ When reviewing or auditing existing code │
│ bugfix │ Specialized bug fixing process │ When fixing bugs or debugging            │
│ build  │ Feature building process       │ When building new features or components │

Magic (special capabilities):
│ deploy-site      │ Put a site online quickly                │
│ visual-explainer │ Create visual documentation and diagrams │
```
Operator confirmed 3 rules + 2 magic capabilities loaded from pollypm/defaults/. Plugins load automatically at startup via the control prompt's manifest sections.
