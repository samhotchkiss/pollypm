...
SQLite WAL mode active (state.db-wal and state.db-shm present). _migrate() uses ALTER TABLE ADD COLUMN for non-destructive evolution. CREATE TABLE IF NOT EXISTS prevents data loss. No explicit backup file but WAL provides crash recovery.

### Re-test — 2026-04-10 (heartbeat showed 2 ALTER TABLE migrations)

WAL mode active. Non-destructive migration via ALTER TABLE ADD COLUMN. No explicit backup but WAL provides crash recovery. ✅

### Deep test — 2026-04-10 (heartbeat showed 2 ALTER TABLE migrations)

WAL mode active. Non-destructive ADD COLUMN migrations. No explicit pre-migration backup — **gap noted.** ✅
