...
Navigated to Settings (press 's'). Both accounts visible in the account list TUI:
```
Key                   Email          Provider  Login
claude_claude_swh_me  claude@swh.me  claude    yes
codex_s_swh_me        s@swh.me       codex     yes
```

Codex account verified in-session: worker ran `echo $CODEX_HOME` → `/Users/sam/.pollypm/homes/codex_s_swh_me/.codex`. Home is isolated, worker uses file-based auth.
