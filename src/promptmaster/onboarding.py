...
from promptmaster.runtime_env import provider_profile_env_for_provider
...
def _runtime_home(root_dir: Path, provider: ProviderKind, index: int) -> Path:
    return root_dir / ".promptmaster" / "homes" / f"onboarding_{provider.value}_{index}"
...
def _final_account_home(root_dir: Path, account_name: str) -> Path:
    return root_dir / ".promptmaster" / "homes" / account_name
...
def _recover_existing_accounts(root_dir: Path) -> dict[str, ConnectedAccount]:
    homes_dir = root_dir / ".promptmaster" / "homes"
...
def _run_login_window(
    ...
    *,
    ...
    temp_session = f"promptmaster-login-{window_label}"
...
def _resolve_account_identifier(config: PromptMasterConfig, identifier: str) -> tuple[str, AccountConfig]:
...
def build_onboarded_config(
    *,
    ...
    base_dir = root_dir / ".promptmaster"
...
def run_onboarding(config_path: Path = DEFAULT_CONFIG_PATH, force: bool = False) -> Path:
    from promptmaster.onboarding_tui import run_onboarding_app
