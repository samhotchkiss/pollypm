...
Thread states: open -> threaded -> waiting-on-pa -> waiting-on-pm -> resolved -> closed. Forward-only transitions enforced. closed->open correctly rejected with ValueError. A dedicated reopen mechanism would be needed for full T058 functionality.

### Re-test — 2026-04-10

Attempted closed→open transition. System raised ValueError: 'Illegal inbox state transition: closed -> open'. Forward-only enforcement working. No reopen mechanism exists yet. **Gap noted.**

### Deep test — 2026-04-10

Thread state machine enforces forward-only transitions. closed→open rejected with ValueError. No reopen mechanism exists. **Gap: reopening closed threads is not supported.**
