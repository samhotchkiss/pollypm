...
def add_account_via_login(config_path: Path, provider: ProviderKind) -> tuple[str, str]:
    ...
    home = root_dir / ".promptmaster" / "homes" / f"ad-hoc-{provider.value}-{next_index}"
    ...
    final_home = root_dir / ".promptmaster" / "homes" / key
...
def remove_account(config_path: Path, identifier: str, *, delete_home: bool = False) -> tuple[str, str]:
    ...
    if config.promptmaster.controller_account == account_name:
        config.promptmaster.controller_account = next(iter(config.accounts), "")
    config.promptmaster.failover_accounts = [
        name for name in config.promptmaster.failover_accounts if name != account_name
    ]
    write_config(config, path=config_path, force=True)
...
def set_controller_account(config_path: Path, identifier: str) -> tuple[str, str]:
    config = load_config(config_path)
    account_name, account = _resolve_account_identifier(config, identifier)
    previous = config.promptmaster.controller_account
    config.promptmaster.controller_account = account_name
    ...
    failover = [name for name in config.promptmaster.failover_accounts if name != account_name]
    ...
    config.promptmaster.failover_accounts = failover
    config.promptmaster.failover_enabled = bool(failover)
    write_config(config, path=config_path, force=True)
    return account_name, account.email or account_name
...
def set_open_permissions_default(config_path: Path, enabled: bool) -> bool:
    config = load_config(config_path)
    config.promptmaster.open_permissions_by_default = enabled
...
def toggle_failover_account(config_path: Path, identifier: str) -> tuple[str, bool]:
    ...
    failover = list(config.promptmaster.failover_accounts)
    enabled: bool
