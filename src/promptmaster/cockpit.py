...
def build_cockpit_detail(config_path: Path, kind: str, target: str | None = None) -> str:
    ...
    if kind == "settings":
        ...
        lines = [
            ...
            f"Control account: {config.promptmaster.controller_account}",
            f"Failover order: {', '.join(config.promptmaster.failover_accounts) or 'none'}",
            f"Open permissions by default: {'on' if config.promptmaster.open_permissions_by_default else 'off'}",
            "",
