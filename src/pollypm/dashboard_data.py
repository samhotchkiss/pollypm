...
def _user_waiting_task_ids_across_projects(
    ...
    Delegates to the work-task state facade so dashboard rendering does
    not own SQLite connection or schema details. Used to suppress
    ``stuck_on_task:<id>`` alerts that are already covered by the
    project's user-waiting status.
    """
