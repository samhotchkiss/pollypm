...
build_recovery_prompt takes provider parameter (ProviderKind.CLAUDE or CODEX). render() method in RecoveryPrompt formats sections into provider-appropriate text. Same checkpoint data reformatted per provider.

### Re-test — 2026-04-10 (confirmed via heartbeat failover explanation)

build_recovery_prompt takes provider param. Same checkpoint data reformatted per provider. Cross-provider recovery changes the provider arg. ✅

### Deep test — 2026-04-10

build_recovery_prompt takes provider param for cross-provider formatting. ✅
