...
def polly_prompt() -> str:
    return (
        ...
        "Run `pm status` every few minutes to check worker states. For any worker that is idle or "
        "waiting_on_user, review their recent output (`git -C <project_path> log --oneline -5`), "
        "then send the next round of instructions via `pm send`. Keep the cycle going: "
        "delegate → monitor → review → redirect → repeat. Never let all workers sit idle at once. "
