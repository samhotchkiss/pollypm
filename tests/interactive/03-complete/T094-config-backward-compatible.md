...
Config uses TOML with optional fields. New fields get defaults (e.g., scheduler_backend defaults to inline). _migrate() in state.py handles schema evolution with ALTER TABLE ADD COLUMN. Old configs load without error.

### Re-test — 2026-04-10 (operator loaded pollypm.toml successfully)

2 accounts, 3 sessions, 5 projects parsed from existing config. ✅

### Deep test — 2026-04-10 (operator loaded config)

pollypm.toml loaded: 2 accounts, 3 sessions, 5 projects. All optional fields default correctly. ✅
