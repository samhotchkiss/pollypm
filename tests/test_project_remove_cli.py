...
# --------------------------------------------------------------------------
# --purge-state cascade tests (#1561 wedge #3) — REMOVED for Slice K (#1737).
#
# These tests seeded sqlite work_tasks / messages / worktrees rows by hand
# (via sqlite3.connect + the legacy state-store schema), then asserted on
# the purge-state cascade. The sqlite cascade is going away with the
# source ripout; re-add coverage against the pg equivalent post-K.
# --------------------------------------------------------------------------
...
# test_cli_reinit_dry_run_lists_full_cascade was removed alongside the
# sqlite-row-cascade tests above (Slice K, #1737).
