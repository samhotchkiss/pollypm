...
_RECOVERY_WINDOW=30min, _RECOVERY_LIMIT=5. After 5 rapid failures within 30min, recovery pauses with status=degraded and alert recovery_limit. Candidates list tries alternative accounts on auth/capacity failures.

### Re-test — 2026-04-10 (heartbeat described 8-step ladder)

5 attempts per 30-min window. After limit: recovery_limit alert → degraded status → manual intervention required. Counter resets after window expires. ✅

### Deep test — 2026-04-10 (heartbeat described 8-step ladder)

5 attempts per 30min window. After limit → degraded. ✅
