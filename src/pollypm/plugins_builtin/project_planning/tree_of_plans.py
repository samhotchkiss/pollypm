...
def decompose_stage_prompt() -> str:
    ...
    return (
        ...
        "- `## Live-surface shipping` — if the user's success condition "
        "is visible on a deployed URL/app/site, include a terminal "
        "`Ship and verify live` module that depends on every build "
        "module. Its acceptance criteria must cover branch integration, "
        "the established deploy command, and a live fetch/marker check. "
        "If the project lacks a documented deploy path, include a small "
        "discover-deploy module before the ship module.\n"
