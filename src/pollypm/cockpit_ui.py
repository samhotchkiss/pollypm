...
from pollypm.cockpit_apps.dashboard import PollyDashboardApp  # noqa: F401
...
from pollypm.cockpit_rail_listview import (  # noqa: F401  (re-exported)
    _RailListView,
)
...
# Re-export the extracted settings-gather helpers so existing import paths
# (and ``monkeypatch.setattr('pollypm.cockpit_ui._collect_recent_tasks_by_account', ...)``
# style hooks in tests) keep working. See #1354 for the rolling cockpit_ui split.
from pollypm.cockpit_settings_gather import (  # noqa: E402,F401
    _GLOBAL_SETTINGS_ROLE_KEYS,
    _ROLE_LABELS,
    _build_settings_role_rows,
    _budget_fields_from_cached_usage,
    _budget_level,
    _collect_recent_tasks_by_account,
    _format_recent_task,
    _gather_settings_data,
    _humanize_bytes,
    _resolved_assignment_from_row,
    _role_assignment_summary,
    _role_label,
    _role_source_style,
    _role_source_text,
    _settings_dir_size,
    _settings_session_refs_by_account,
    _settings_status_dot,
)
...
# Re-export the inbox row-formatting + plan-review helpers extracted to
# ``cockpit_ui_inbox_format`` so existing import paths
# (and ``monkeypatch.setattr('pollypm.cockpit_ui._format_inbox_thread_row', ...)``
# style hooks in tests) keep working. See #1354 for the rolling cockpit_ui split.
from pollypm.cockpit_ui_inbox_format import (  # noqa: E402,F401
    _INBOX_DEFAULT_LENS_SLUG,
    _INBOX_LENS_KINDS,
    _INBOX_LENSES,
    _INBOX_PRIORITY_RANK,
    _archive_success_message,
    _build_pm_context_line,
    _build_plan_review_denial_primer,
    _build_plan_review_primer,
    _extract_blocking_question_meta,
    _extract_plan_judgment_calls,
    _extract_plan_review_meta,
    _extract_plan_summary_block,
    _extract_proposal_spec,
    _format_inbox_plan_review_row,
    _format_inbox_reply_row,
    _format_inbox_row,
    ...
    _plan_review_message_body_for_display,
    _project_pm_label,
    _project_pm_persona,
    _render_heuristic_action_block,
    _render_inbox_triage_banner,
    _render_user_prompt_block,
    _resolve_pm_target,
    _task_is_rollup,
    _task_recent_timestamp,
    _triage_bucket,
    _triage_label,
    _truncate_plan_summary,
)
