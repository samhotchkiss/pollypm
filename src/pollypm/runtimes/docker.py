...
class DockerRuntimeAdapter:
    ...
    def wrap_command_exec(
        ...
        if not account.docker_image:
            raise ValueError(
                f"Account {account.name} is missing docker_image for docker runtime"
            )
        if account.home is None:
            raise ValueError(
                f"Account {account.name} needs a persistent home for docker runtime"
            )
        ...
        if command.resume_argv and command.resume_marker is not None:
            ...
            if command.fresh_launch_marker is not None:
                inner_parts.append(
                    f"rm -f {shlex.quote(str(self._container_path_for_home_file(account, command.fresh_launch_marker)))}"
                )
            if "CODEX_HOME" in env:
                inner_parts.append(
                    self._fresh_codex_cleanup(env["CODEX_HOME"], quoted_resume_marker)
                )
            inner_parts.append(
                f"if [ -f {quoted_resume_marker} ]; then {shlex.join(command.resume_argv)}; "
                f"_pm_status=$?; if [ $_pm_status -eq 0 ]; then exit 0; fi; fi"
            ...
            if command.fresh_launch_marker is not None:
                inner_parts.append(
                    f"touch {shlex.quote(str(self._container_path_for_home_file(account, command.fresh_launch_marker)))}"
                )
            inner_parts.append(f"exec {shlex.join(command.argv)}")
        else:
            if command.fresh_launch_marker is not None:
        ...
        docker_parts.append(account.docker_image)
        docker_parts.extend(["sh", "-lc", inner])
        return WrappedRuntimeCommand(argv=docker_parts, cwd=project.root_dir)
