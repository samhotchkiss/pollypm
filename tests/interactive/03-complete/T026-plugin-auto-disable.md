...
Heartbeat classifies sessions. After N failures (5 in 30min window), status becomes degraded and recovery_limit alert fires. Effectively auto-disables problematic sessions.

### Re-test — 2026-04-10 (heartbeat described escalation)

After 5 failures in 30min: recovery_limit alert → degraded status. Session effectively auto-disabled until manual intervention. ✅

### Deep test — 2026-04-10 (heartbeat described escalation)

After 5 failures in 30min window: recovery_limit alert → degraded status. Effectively disables session until manual intervention. ✅
